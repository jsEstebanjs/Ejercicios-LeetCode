var checkValid = function(matrix) {
    for(let i =0; i<matrix.length;i++){
        let longitudInicial = [...matrix]
        let item = new Set([...matrix[i]])
        item =[...item]
        let itemsY = [];

         if(item.length !== longitudInicial.length){
            return false
            }

    for(let x = 0;x<matrix[i].length;x++){
        itemsY.push(matrix[x][i]);

        //comparacion Y
        if(itemsY.length === longitudInicial.length){
            itemsY = new Set([...itemsY])
            itemsY = [...itemsY];
